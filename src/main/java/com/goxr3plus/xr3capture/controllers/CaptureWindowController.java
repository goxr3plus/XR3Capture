...
public class CaptureWindowController extends Stage {
	...
	/**
	 * When a key is being pressed into the capture window then this Animation Timer
	 * is doing it's magic.
	 */
	...
	/**
	 * This AnimationTimer waits until the canvas is cleared before it can capture
	 * the screen.
	 */
	...
	public void addControllerReferences(final MainWindowController mainWindowController,
	...
	public void initialize() {
		...
		// Scene
		final Scene scene = new Scene(stackPane, data.screenWidth, data.screenHeight, Color.TRANSPARENT);
	...
	protected void repaintCanvas() {
		...
		// gc.fillRect(model.rectUpperLeftX, model.rectUpperLeftY,model.rectWidth,
		// model.rectHeight)
		...
		if (!data.hideExtraFeatures.getValue()) {
			...
			// Show the Size
			final double middle = data.rectUpperLeftX + data.rectWidth / 2.00;
			gc.setLineWidth(1);
			// gc.setStroke(Color.FIREBRICK);
			// gc.strokeRect(middle - 78,
			// model.rectUpperLeftY < 25 ? model.rectUpperLeftY + 2 : model.rectUpperLeftY -
			// 26.00, 79, 25);
	...
	public void prepareForCapture() {
		...
		// if (settingsWindowController.getMarryTTSToggle().isSelected())
		// Main.textToSpeech.speak("Select an area of the screen dragging your mouse and
		// then press Enter or Space");
	}

	/**
	 * Return an array witch contains the (UPPER_LEFT) Point2D of the rectangle and
	...
	/**
	 * The work of the Service is to capture the Image based on the rectangle that
	 * user drawn of the Screen.
	 *
	 * @author GOXR3PLUS
	 */
	public class CaptureService extends Service<Boolean> {
		...
		public void startService(final BufferedImage image2) {
			...
			final File file = fileSaver.get().showSaveDialog(CaptureWindowController.this);
			if (file == null)
				repaintCanvas();
			else {
		...
		private void done() {
			...
			// Was it successful?
			if (!getValue())
				ActionTool.showNotification("Error", "Failed to capture the Screen!", Duration.millis(2000),
