...
public class CaptureWindowController extends Stage {
	...
	private void initialize() {
		...
		Media media = null;
		try {
			media = new Media(getClass().getResource("/video/video.mp4").toURI().toString());
	...
	public void createImage() {
		...
		countingThread = new Thread(() -> {
			...
			// CountDown
			if (!Main.applicationController.getTimeSlider().isDisabled()) {
				for (i = (int) Main.applicationController.getTimeSlider().getValue(); i > 0; i--) {
				...
				// !interrupted?
				if (Main.settingsController.getAkbarTrollToggle().isSelected() && !interrupted) {
					...
					try {
						...
						// Clear the canvas
						Platform.runLater(() -> gc.clearRect(0, 0, getWidth(), getHeight()));
						...
						mediaPlayer.stop();
						mediaView.setVisible(false);
						Main.textToSpeech.setVoice("dfki-poppy-hsmm");
					}
				}
			}
